...
def _full_angle(angle, hooks=2, bodies=2, usps=2, ctas=2):
    return (
        _c(svc.HOOK, angle, "h", n=hooks)
        + _c(svc.BODY, angle, "b", n=bodies)
...
def test_one_missing_type_zeroes_that_angle():
    pool = _c(svc.HOOK, A1, n=5) + _c(svc.BODY, A1, n=5) + _c(svc.USP_SET, A1, n=5)
...
def test_global_components_count_toward_every_angle():
    ...
    pool = (
        _c(svc.HOOK, A1, n=2) + _c(svc.BODY, A1, n=2) + _c(svc.USP_SET, A1, n=2)
        + _c(svc.HOOK, A2, n=2) + _c(svc.BODY, A2, n=2) + _c(svc.USP_SET, A2, n=2)
...
def test_next_best_points_at_the_highest_leverage_slot():
    """3 hooks x 1 body x 2 usp x 2 cta = 12. Adding a BODY doubles it (+12);
    adding a HOOK adds only 4. The operator must be told BODY."""
    pool = (
        _c(svc.HOOK, A1, "h", n=3) + _c(svc.BODY, A1, "b", n=1)
        + _c(svc.USP_SET, A1, "u", n=2) + _c(svc.CTA, A1, "c", n=2)
    )
    cap = svc.pool_capacity(pool, [A1])
    assert cap["total_combinations"] == 12
    assert cap["per_angle"][0]["next_best_type"] == svc.BODY
    assert cap["next_best"]["component_type"] == svc.BODY
...
def test_blocked_angle_reports_zero_marginal_gain_for_non_blocking_types():
    ...
    pool = _c(svc.HOOK, A1, n=2) + _c(svc.BODY, A1, n=2) + _c(svc.USP_SET, A1, n=2)
...
def test_unknown_component_types_are_ignored():
    pool = _full_angle(A1) + [
        {"component_type": "SUBHOOK", "angle_key": A1, "content": "x",
...
def test_worked_example_from_the_architecture_doc():
    ...
    8 hooks x 5 bodies x 4 usp_sets x 5 ctas = 800, x4 angles x6 formulas."""
    angles = [f"ang_{i:012d}" for i in range(4)]
    pool = []
    for a in angles:
        pool += _c(svc.HOOK, a, "h", n=8) + _c(svc.BODY, a, "b", n=5)
