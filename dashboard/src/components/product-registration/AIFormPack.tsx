...
export default function AIFormPack({
	onComplete,
	setIsProcessing,
	isProcessing,
}: Props) {
	const [selectedFile, setSelectedFile] = useState<File | null>(null);
	const [importResult, setImportResult] = useState<AIFormImportResponse | null>(
		null,
	);
	...
	const handleDownloadTemplate = async () => {
		try {
			const data = await fetchAPI<{ filename: string; content: string }>(
				"/api/product-knowledge/ai-form-template",
			);
	...
	const handleCopyPrompt = async () => {
		try {
			const data = await fetchAPI<{ prompt: string }>(
				"/api/product-knowledge/ai-coaching-prompt",
			);
	...
	const handleUpload = async () => {
		...
		try {
			const result = await postMultipartAPI<AIFormImportResponse>(
				"/api/product-knowledge/import-ai-form",
				formData,
			);
		...
		<div className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden mb-8">
			<div className="p-6 border-b border-slate-800 bg-slate-800/20">
				...
				<p className="text-sm text-slate-400 mt-2">
					Don't have all the facts? Let an AI assistant help you interview and
					fill the form.
				</p>
			</div>
					...
					<div className="space-y-4 p-5 rounded-xl bg-slate-800/30 border border-slate-700/50">
						...
						<div className="space-y-4">
							<input
								...
								accept=".md,.markdown,.json,.JSON,.txt"
							...
							<p className="text-[11px] leading-relaxed text-slate-500">
								...
								<span className="text-slate-300">
									`.md`, `.markdown`, `.json`, `.JSON`, `.txt`
								</span>
								. Raw JSON, fenced `json` markdown, UTF-8 BOM, and wrapped
								single-object JSON are accepted.
							</p>
							<button
				...
				{importResult && (
					<div
						className={`p-5 rounded-xl border ${
							importResult.parse_status === "PARSED"
					...
					>
						<div className="flex items-center justify-between mb-4">
							<h4 className="text-sm font-bold text-white uppercase tracking-wider">
								Import Status: {importResult.parse_status}
							</h4>
						...
						{importResult.parse_errors.length > 0 && (
							<div className="mb-4 space-y-1">
								{importResult.parse_error_code && (
									<div className="text-[11px] text-red-300 font-bold uppercase tracking-wider">
										Error Code: {importResult.parse_error_code}
									</div>
								)}
								{importResult.parse_error_detail && (
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4 text-[11px]">
							<div className="rounded-lg border border-slate-700/50 bg-slate-900/40 p-3">
								<div className="text-slate-500 uppercase tracking-widest text-[10px] font-bold mb-1">
									Parser Strategy
								</div>
								<div className="text-slate-200">
									{importResult.parser_strategy_used || "NOT_PARSED"}
								</div>
							</div>
							<div className="rounded-lg border border-slate-700/50 bg-slate-900/40 p-3">
								<div className="text-slate-500 uppercase tracking-widest text-[10px] font-bold mb-1">
								...
								<div className="text-slate-200">
									ext={importResult.detected_extension || "unknown"} | type=
									{importResult.detected_content_type || "unknown"}
								</div>
							</div>
						</div>

						{importResult.parse_status !== "PARSED" &&
							importResult.accepted_formats.length > 0 && (
								<div className="mb-4 rounded-lg border border-slate-700/50 bg-slate-900/40 p-3">
									<div className="text-slate-500 uppercase tracking-widest text-[10px] font-bold mb-2">
										Accepted Formats
									</div>
									<ul className="space-y-1">
										{importResult.accepted_formats.map((format, i) => (
						...
						{importResult.parse_warnings.length > 0 && (
							<div className="mb-4 space-y-1">
								{importResult.parse_warnings.map((warn, i) => (
									<div
										key={i}
										className="text-xs text-amber-400 flex items-center gap-2"
									>
							...
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-slate-700/50">
								<div>
									...
									<div className="text-sm text-white truncate">
										{importResult.parsed_request.product_name}
									</div>
								</div>
								<div>
									<div className="text-[10px] text-slate-500 uppercase font-bold">
									...
									<div className="text-sm text-white">
										{importResult.parsed_request.source_lane}
									</div>
								</div>
								<div>
									<div className="text-[10px] text-slate-500 uppercase font-bold">
									...
									<div className="text-sm text-white">
										{importResult.user_review_required ? "YES" : "NO"}
									</div>
								</div>
								<div>
									<div className="text-[10px] text-slate-500 uppercase font-bold">
									...
									<div className="text-sm text-white truncate text-[10px]">
										{importResult.write_back_status}
									</div>
								</div>
							</div>
						)}
						...
						{importResult.parse_status === "PARSED" &&
							importResult.completion_response && (
								<div className="mt-4 rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-4">
									<div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
										<div>
											<div className="text-[10px] text-emerald-300 uppercase tracking-widest font-bold">
												Smart Completion Ready
											</div>
											<div className="text-sm text-white">
												Claim Gate:{" "}
												{importResult.completion_response.claim_gate}
												{importResult.completion_response.claim_tokens.length >
													0 && (
													<span className="text-slate-400">
														{" "}
														| tokens=
														{importResult.completion_response.claim_tokens.join(
															", ",
														)}
													</span>
												)}
