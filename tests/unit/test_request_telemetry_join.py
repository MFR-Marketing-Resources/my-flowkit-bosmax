...
@pytest.mark.asyncio
async def test_recursion_guard_failure_is_visible_in_request_snapshot():
    db = await get_db()
    request_id = "navigation_resume_recursion_guard_001"
    now = crud._now()
    ...
    await crud.upsert_request_telemetry(
        ...
        error_message="ERR_FLOW_RESUME_RECURSION_GUARD - resume attempt limit exceeded",
        failed_at=now,
    )
    ...
    assert request["status"] == "FAILED"
    assert request["error_message"].startswith("ERR_FLOW_RESUME_RECURSION_GUARD")
