...
from agent.api.copy_components import router as copy_components_router
...
from agent.api.copy_bulk import router as copy_bulk_router
...
from agent.api.product_treatment_factory import router as product_treatment_factory_router
...
app.include_router(copy_bulk_router, prefix="/api")
...
async def health():
    ...
    return {
        "status": "ok",
        # "version" is NOT a build identity. It is a hand-maintained contract number
        # that has never tracked the code, and it was read as provenance by an audit
        # which then concluded the system had no build observability at all. Keep the
        # key (consumers may read it) but say what it is, and name the endpoints that
        # DO answer identity so the next reader does not repeat the mistake.
        "version": "0.2.0",
        "version_is_build_identity": False,
        "identity_endpoints": [
            "/api/local-agent/version-proof",
            "/api/operator/runtime-storage-status",
            "/api/flow/bind-check",
        ],
        # Extension provenance, already in scope from the get_status() above — no
        # extra I/O. build_sha is derived from extension/build-stamp.js (generated by
        # scripts/stamp-extension-build.mjs); it is None on an unstamped folder, which
        # is honest. build_dirty=True means the loaded folder has uncommitted changes
        # and therefore does NOT correspond to build_sha.
        "extension_build_sha": extension_status.get("build_sha"),
        "extension_build_stamped": bool(extension_status.get("build_stamped")),
        "extension_build_dirty": extension_status.get("build_dirty"),
        "extension_build_branch": extension_status.get("build_branch"),
