	...
	list.innerHTML = entries
		.map((entry, i) => {
           ...
           <div class="detail-value detail-error">${escHtml(error)}</div>
			...
			const hasDetails =
				entry.url || entry.payloadSummary || entry.responseSummary || error;
...
document.querySelectorAll("[data-dashboard-route]").forEach((button) => {
	button.addEventListener("click", () => {
		const routeKey = button.getAttribute("data-dashboard-route");
		const url = routeKey ? DASHBOARD_ROUTES[routeKey] : "";
		if (!url) return;
