...
def process_one(con, pid, budget):
    ...
    # 4. governed absences. INGREDIENTS + WARNINGS are strict-evidence fields: an AI proposal is
    # NOT acquired evidence (no valid provenance), so for these debt products they are ALWAYS
    # cleared and dispositioned SOURCE_UNAVAILABLE — never an AI-asserted ingredient/warning fact.
    # USAGE may remain a safe product-type SUPPORTED_INFERENCE; only dispositioned if still empty.
        ...
        if ne(d.get(fld)):
            call("PATCH", f"/product-intelligence/review-drafts/{did}",
                 {fld: "", "reviewed_by": REVIEWER,
                  "reviewer_note": "PI-12: ingredients/warnings require acquired evidence; an AI "
                                   "proposal is not valid provenance -> governed absence."})
...
def main():
    ...
    for n, pid in enumerate(todo, 1):
        # reopen a fresh RO connection each item so we see committed writes
