...
_TYPE_PRIORITY = {
    ...
    "GENERATE_VIDEO": 2, "REGENERATE_VIDEO": 2, "GENERATE_VIDEO_REFS": 2, "TRUE_F2V": 2,
    "UPSCALE_VIDEO": 3,
}

# Backward-compatible module state for direct test calls into _handle_failure.
...
async def _prerequisites_met(req: dict, orientation: str) -> bool:
    ...
    if req_type in ("GENERATE_VIDEO", "REGENERATE_VIDEO", "GENERATE_VIDEO_REFS", "TRUE_F2V", "UPSCALE_VIDEO"):
        scene = await crud.get_scene(req.get("scene_id"))
        if not scene:
            return True  # let _dispatch handle "scene not found"
        if req_type in ("GENERATE_VIDEO", "REGENERATE_VIDEO", "GENERATE_VIDEO_REFS", "TRUE_F2V"):
    ...
    if await _is_already_completed(req, orientation):
        ...
        if req_type in ("GENERATE_CHARACTER_IMAGE", "REGENERATE_CHARACTER_IMAGE", "EDIT_CHARACTER_IMAGE"):
            ...
            if scene:
                ...
                elif req_type in ("GENERATE_VIDEO", "REGENERATE_VIDEO", "GENERATE_VIDEO_REFS", "TRUE_F2V"):
...
async def _is_already_completed(req: dict, orientation: str) -> bool:
    ...
    if req_type in ("GENERATE_VIDEO", "GENERATE_VIDEO_REFS", "TRUE_F2V"):
