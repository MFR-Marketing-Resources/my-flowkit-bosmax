...
def test_forensic_runtime_audit_contract_is_wired_to_backend_extension_and_import_simulation():
    background_source = _read("extension/background.js")
    dom_source = _read("extension/content-flow-dom.js")
    audit_script = _read("scripts/forensic-f2v-runtime-audit.ps1")
    import_helper = _read("scripts/forensic-import-simulation.js")
    ...
    for token in [
        "prompt_field_found",
        "bottom_composer_config_pill_visible",
        "bottom_composer_config_pill_text",
        "const statusPayload = resp?.data && typeof resp.data === 'object' ? resp.data : resp;",
        "background_status_missing build=${backgroundBuildId || 'legacy-compatible'}",
    ]:
        assert token in dom_source
    ...
    for token in [
        "BACKEND_PROCESS",
        "RUNNER_SELF_TEST",
        "READY_FOR_SINGLE_LIVE_F2V_UAT",
        "PRODUCT_ID_REQUIRED_FOR_PRODUCT_READY_PROOF",
        "forensic-import-simulation.js",
    ]:
        assert token in audit_script

    for token in [
        "__BOSMAX_F2V_FLOW_QUEUE_RUNNER__",
        "background_import_ok_in_source",
        "import_ok_logged",
        "import_block_files",
    ]:
