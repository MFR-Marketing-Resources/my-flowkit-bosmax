...
export function usePosterGuidedWorkflow(): PosterGuidedWorkflow {
	...
	const restoreFromReopen = useCallback(
		(recon: PosterDeliverableReconstruction, p: Product | null) => {
			...
			setCreativeModeState(String((recon.render_manifest?.provenance as { creative_mode?: string } | undefined)?.creative_mode ?? ""));
	...
	const generateScene = useCallback(async () => {
		...
		try {
			const promptPackage = await createPosterPromptDraft({
				product_id: product.id,
				poster_objective: objectiveText || approvedCopySet.objective || "Poster",
			...
			if (!promptPackage.generation_allowed || !promptPackage.poster_prompt) {
				throw new Error(
					promptPackage.blocked_reasons?.[0] ||
						"Produk belum dibenarkan untuk jana scene poster.",
				);
			}

			setSceneGenerationStage("validating_product");
			const gate = await resolveExactGenerationGate(product.id);
			if (token !== sceneGenerationTokenRef.current) return;
			...
			if (exactComposite) {
				...
				const sceneOnly = await buildExactSceneOnlyPrompt(
					product.id,
					prompt,
				);
				if (token !== sceneGenerationTokenRef.current) return;
			...
			if (!(status === "DONE" || status === "COMPLETED") || !job.media_id) {
				throw new Error(
					job.error ||
						`Penjanaan scene tamat sebagai ${job.status || "UNKNOWN"} tanpa imej.`,
				);
			}

			// The poster compose endpoint owns the exact-product deterministic layer.
			// Keep the raw scene plate here; passing an already composited exact output
			// would insert the canonical product twice during poster composition.
			const mediaId = job.media_id;
			const url = job.url || `/api/flow/retrieved/${mediaId}`;
			if (!mediaId) throw new Error("Penjanaan scene tidak memulangkan media_id.");
			...
			setSceneGenerationError(
				friendlyError(
					...
					"Gagal menyediakan scene poster. Tiada poster dikompos sehingga scene berjaya.",
				),
			);
		} finally {
