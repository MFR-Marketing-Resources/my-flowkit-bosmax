...
export default function CopySetRegistryPage() {
	...
	const openActivationReportFor = (blueprint: CopyBlueprintV2Record) => {
		navigate(
			`/reporting/operations?section=copy-authority-activation&blueprint_id=${encodeURIComponent(blueprint.blueprint_id)}`,
		);
	};
	...
	return (
		<div className="mx-auto max-w-6xl space-y-6 p-4 md:p-8" data-testid="copy-set-registry-page">
			...
			{selectedProduct && activeTab === "LIBRARY" && reviewTarget ? (
				...
				<Section title="Review and approve" helper="Pick a blueprint from the library below, then review or approve it here. Bulk activation is managed in Reporting. Approved text is immutable.">
							...
							{reviewableBlueprint ? (
								...
								<div className="mt-4 rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-4">
									{canActivateAuthority(reviewTarget) || activatedBlueprintId === reviewTarget.blueprint_id ? (
										<>
											<HelperText className="text-emerald-300/80">Approved copy is immutable. Activation is owner-gated and managed in the Reporting view.</HelperText>
											<button type="button" data-testid="open-activation-report-from-review" disabled={busy} onClick={() => openActivationReportFor(reviewTarget)} className="mt-3 rounded-xl border border-blue-500/40 bg-blue-600/20 px-4 py-2 text-xs font-bold uppercase text-blue-100 disabled:opacity-40">{activatedBlueprintId === reviewTarget.blueprint_id ? "VIEW ACTIVATION REPORT" : "OPEN ACTIVATION REPORT"}</button>
										</>
									) : (
						...
						<div className="grid gap-4 sm:grid-cols-2">
							{paginatedLibraryBlueprints.map((bp) => {
								...
								if (isCurrent && canActivate) {
									...
									cardLabel = "Open activation report";
									cardDisabled = busy;
									onCardAction = () => openActivationReportFor(bp);
