# BOSMAX Dual-Route Multi-Mode Temporal Extension Contract v0.2
...
| `reviewed_base_sha` | `9c038a0acc15e14556c7710e202e446132f6b232` |
| `reviewed_remote` | `origin/main` |
| `repo_anchor` | `C:\Users\USER\Desktop\_ref_flowkit` |

## 1A. v0.2 Changelog

- This `v0.2` patch applies the Codex forensic review amendments raised after the `v0.1` draft review.
- This patch remains document-only. No application code, extension runtime logic, schema, package, or migration work is authorized by this file.
- `NO_CODING_AUTHORIZED` remains in force.
...
- The current checkout already contains operator API or input slots for avatar, headwear style, camera style, scene context, language, trigger, silo, and formula.
  Evidence: `agent/api/operator.py`.
- The current checkout already has an implemented 9-section prompt compiler service in addition to the contract doc.
  Evidence: `agent/services/prompt_compiler_9_section.py`, `docs/contracts/PROMPT_COMPILER_9_SECTION_CONTRACT.md`.
...
- Actual operator registry datasets for avatar, headwear, scene, camera, trigger, silo, and formula values depend on external operator-pack files and are not source-controlled in this repo.
...
- `PRODUCT`
  - VERIFIED FROM REPO for product row and product image asset fields.
  - NOT VERIFIED YET for fully implemented product upload-to-Flow media proof.
  - `agent/services/product_intelligence.py` currently returns `UPLOAD_PENDING_IMPLEMENTATION` from `upload_product_to_flow`, so product media handoff must not be stated as fully implemented.
- `SCENE`
...
5. Infer compatible character pattern, scene pattern, and copy angle from verified truth where repo-backed inference exists.
...
- Scene, camera, and copy profile inference is PARTIALLY VERIFIED through `product_mapping.py`, `product_intelligence.py`, `product_preflight.py`, and `product_creative_brief.py`.

### Inference boundaries

- Character inference from product truth is NOT VERIFIED YET. The repo has avatar/operator concepts, but automatic population of ethnicity, gender, age band, or full character persona from product truth is not proven as current runtime behavior.
...
- VERIFIED FROM REPO for audited UI workspaces:
  - `T2V`, `F2V`, `I2V`, and `IMG` workspace modules exist under `dashboard/src/components/workspace/`.
- VERIFIED FROM REPO for API-level operator blueprint inputs:
  - product selection
  - avatar selection
  ...
  - formula selection
  - language defaults
  - trigger and silo inputs
- NOT VERIFIED YET for external operator-pack-driven registries:
  - the actual governed registry datasets that populate those input slots are external to source control in this checkout.
- PARTIALLY VERIFIED:
...
- VERIFIED FROM REPO: `I2VModule` exposes generic `subjectAsset`, `sceneAsset`, and `styleAsset`.
...
- NOT VERIFIED YET: a dedicated product-slot composition factory inside Ingredients mode.
- PROPOSED / NOT VERIFIED: a character-holding-product planner or governed asset-combination factory.
...
- VERIFIED FROM REPO: `IMGModule` already accepts generic subject, scene, and style assets plus prompt text.
- NOT VERIFIED YET: a dedicated product-slot composition factory in the current image workflow.
- PROPOSED / NOT VERIFIED: a governed image planner that explicitly distinguishes character-asset creation, character-holding-product composition, product lifestyle generation, and scene-reference generation as separate planning intents.
...
- Contradiction note: current operator blueprint logic uses `4` scenes for durations `<=30s` and `8` scenes for durations `>30s` in `agent/api/operator.py`. Therefore `target_duration / 8` block planning is a proposed planning rule only, not current repo behavior.
- Engine profile must override duration math where the runtime engine uses different scene or block rules.
...
- VERIFIED FROM REPO: prompt insertion, asset verification, generation-start telemetry, batch surfaces, and smoke-execution surfaces exist.
- NOT VERIFIED YET: render completion detection, selected clip continuation, Extend button detection, Insert button detection, full per-block continuation orchestration, and per-block recovery loop.
...
- VERIFIED FROM REPO: current batch execution is variant-based, not block-expanded.
- PROPOSED / NOT VERIFIED: `block_index`, per-block retry tracking, per-block output tracking, and per-block resume semantics as first-class batch-planning state.
...
- `Prompt Compiler Core`
  - PARTIALLY VERIFIED through `docs/contracts/PROMPT_COMPILER_9_SECTION_CONTRACT.md` and the implemented service `agent/services/prompt_compiler_9_section.py`.
  - NOT VERIFIED YET for complete validation, strict scrubbing guarantees, or full fail-closed enforcement around internal markers and claim boundaries.
...
Current repo proof gaps that remain missing or unproven:

- full tuple legality validation
- canonical-vs-preview isolation
- strict internal-marker scrub
- enforced product-dimension truth
- enforced continuation or block integrity
