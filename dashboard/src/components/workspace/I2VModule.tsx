    ...
    <div className="flex h-full gap-6">
      <div className="flex-1 space-y-6 overflow-y-auto pr-2 pb-12">
        ...
        <div className="pt-4">
          ...
          >
            {isUploading ? 'Uploading Assets...' : isExecuting ? 'Executing Ingredients Sequence...' : 'START GENERATION'}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {['VERTICAL', 'HORIZONTAL'].map(o => (
                  <button key={o} onClick={() => setOrientation(o as any)} className={`py-2 rounded-lg text-[10px] font-bold border transition-all ${orientation === o ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-slate-950 border-slate-800 text-slate-500'}`}>{o === 'VERTICAL' ? '9:16' : '16:9'}</button>
                ))}
              </div>
            </div>
            <div className="space-y-3">
