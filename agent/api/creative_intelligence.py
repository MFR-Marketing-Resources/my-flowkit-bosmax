...
mutation. The seeds only write the config tables ``avatar_product_fit`` /
``creative_scene_prompt``. Scene templates keep ``[AVATAR]``/``[PRODUCT]``
placeholders unresolved and are never sent to generation.
"""
