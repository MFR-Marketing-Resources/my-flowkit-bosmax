...
// MAIN graft: single top-level fetch implementation backing the detached
// RESOLVE_LOCAL_ASSET / RESOLVE_LOCAL_ASSET_RESULT proxy contract.
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
	...
	// Detached ACK/result contract (MAIN graft): sendResponse() fires synchronously,
	// the actual fetch runs after, and the result is pushed back via a separate
	// RESOLVE_LOCAL_ASSET_RESULT message — this must stay outside handleExecuteFlowJob,
	// handleGfv2Job, and generic handleMessage, and ahead of the generic fallback below.
...
async function handleExecuteFlowJob(job) {
	...
	let targetResolution = await resolveFlowExecutionTarget(job);
	if (!targetResolution.ok && targetResolution.error === "ERR_NO_FLOW_TAB") {
		// MAIN-style auto-open/create-project recovery, non-GFV2 path only. GFV2 already
		// returned above via handleGfv2Job and never reaches this fallback.
		...
		if (openResult?.ok) {
			targetResolution = await resolveFlowExecutionTarget(job);
		}
	}
