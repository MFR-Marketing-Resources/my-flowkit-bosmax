...
_COLUMNS = {
    ...
    "product_intelligence_review_draft": {"product_id", "review_status", "product_description", "benefits_json", "usp_json", "hook_angles_json", "cta_angles_json", "pain_points_json", "usage_text", "ingredients_text", "warnings_text", "target_customer_text", "paste_anything_summary", "source_urls_json", "image_evidence_json", "package_notes", "size_or_volume", "product_form_factor", "packaging_description", "product_truth_lock", "claim_gate", "claim_risk_level", "claim_tokens_json", "allowed_claims_json", "blocked_claims_json", "buyer_persona_snapshot_json", "copy_strategy_summary_json", "confidence_score", "completeness_score", "readiness_status", "reviewer_note", "created_by", "reviewed_by", "approved_by", "approved_at", "rejected_by", "rejected_at", "updated_at"},
...
async def bulk_update_bulk_queue_rows(updates: list[dict]) -> int:
    """Apply queue metadata updates as one transaction.

    This helper is intentionally limited to existing queue rows and the
    whitelisted queue columns.  It never changes Product Truth tables.
    """
    if not updates:
        return 0
    db = await get_db()
    ...
    async with _db_lock:
        for item in updates:
            ...
            cursor = await db.execute(
                f"UPDATE fastmoss_bulk_draft_status SET {assignments} WHERE reference_id=?",
                [*values.values(), reference_id],
            )
