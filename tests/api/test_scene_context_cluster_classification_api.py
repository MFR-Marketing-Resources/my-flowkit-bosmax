...
def test_cluster_coverage_api_exposes_deterministic_read_only_contract():
    ...
    assert (body["active_scene_total"], body["classified_scene_total"], body["review_required_scene_total"], body["shared_scene_total"]) == (20, 13, 7, 2)
...
def test_classification_api_hides_prompts_and_has_no_side_effect_contract():
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["active_scene_total"] == 20
    assert body["registry_mutations"] == 0
    assert len(body["scenes"]) == 20
