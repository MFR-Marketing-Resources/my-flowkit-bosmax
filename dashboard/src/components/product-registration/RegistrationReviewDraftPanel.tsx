...
export default function RegistrationReviewDraftPanel({ draft, onClear }: Props) {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="p-4 rounded-xl bg-blue-500/5 border border-blue-500/20 flex items-center gap-3">
        ...
        <p className="text-xs text-blue-300 font-medium">
          Governance: <span className="font-bold">{draft.write_back_status}</span>. Canonical database write-back is disabled in this phase.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {Object.entries(draft.canonical_candidate_fields).map(([key, value]) => {
                ...
                return (
                  <div key={key} className={`flex items-center justify-between p-3 rounded-lg border transition-all ${
                    isApproved ? 'bg-emerald-500/5 border-emerald-500/30' : 'bg-slate-800/30 border-slate-700/50'
                  }`}>
        ...
        <div className="space-y-8">
          ...
          <section className="p-6 rounded-2xl bg-slate-900/50 border border-slate-800">
            ...
            <div className="space-y-4">
              ...
              {draft.human_review_fields.length > 0 && (
                <div className="space-y-2">
                  <span className="text-[10px] font-bold text-amber-500 uppercase">Review Required</span>
                  <div className="flex flex-wrap gap-2">
                    {draft.human_review_fields.map(f => (
                      <span key={f} className="px-2 py-1 rounded bg-amber-500/10 text-[10px] text-amber-400 border border-amber-500/20 font-bold uppercase">
                        {f}
                      </span>
                    ))}
                  </div>
          ...
          <div className="pt-4 flex gap-4">
            ...
            <button
              disabled
              className="flex-1 py-3 px-4 rounded-xl bg-indigo-500/10 text-indigo-400 text-xs font-bold uppercase tracking-widest cursor-not-allowed border border-indigo-500/20"
            >
              Commit to DB (Disabled)
            </button>
          </div>
        </div>
      </div>
